...
- `useKeyboardShortcuts` — attaches a single `keydown` listener on `window` and maps all keyboard shortcuts. Skips events when the target is a text input or when the shortcut overlay is open. Receives `activeTab`, `setActiveTab`, and `toggleValidatorOption` to support tab navigation and tab-scoped shortcuts. Global: `⌥⇧1…5` jump to a tab and `⌥⇧←/→` cycle tabs (checked before the version digits so shifted digits route to tabs, not versions). Generator-scoped (only when `activeTab === "generator"`): `⌥C` cycles export format and `⌥⇧C` copies the whole batch. Validator-scoped: the accept-option keys (`⌥R`, `⌥[`, `⌥-`) only fire when `activeTab === "validator"`.
