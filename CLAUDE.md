...
**Theming:** All design tokens (colour, type, space, radius, motion) are defined in `src/design-system/tokens.css` and imported by `index.css`. Theme-sensitive colours are set under `:root`/`[data-theme="dark"]` and `[data-theme="light"]`; accent palettes are set via `[data-accent="<name>"]`. The living token reference is `src/design-system/Design System.html` — open it directly in a browser. That page is a specimen document, not app UI: its own chrome (28px section titles, a 56px hero demo, 24px letterform tiles) sits deliberately outside the app type ramp, because the page exists to display that ramp rather than obey it. It is listed in `detector.ignoreFiles` in `.impeccable/config.json` for that reason; the ignore is scoped to this one path so a stray 24px in real app CSS is still caught. There is no CSS framework: Tailwind was removed once it was clear no utility class was ever used, and the handful of preflight rules the stylesheet relied on (form-control font inheritance, zeroed heading/paragraph margins, collapsed table borders, a global `line-height: 1.5`) are now authored explicitly at the top of `index.css`. Geist and Geist Mono are self-hosted from `src/design-system/fonts/` (SIL Open Font License 1.1) and declared in `src/design-system/fonts.css`; the app makes no third-party requests at runtime, which is also what lets the container's `font-src 'self'` CSP work.
